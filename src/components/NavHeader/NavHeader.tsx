...
export default function NavHeader() {
  const { t, i18n } = useTranslation(['home', 'cart'])
    ...
    <div className='flex justify-end text-sm'>
      ...
      {isAuthenticated && (
        <Popover
          ...
          renderPopover={
            <div className='relative rounded-sm border border-gray-200 bg-white text-sm capitalize shadow-md'>
              <Link
                to={path.profile}
                className='block w-full bg-white py-3 px-4 text-left hover:bg-slate-100 hover:text-cyan-500'
              >
                {t('home:header.my account')}
              </Link>
              <Link
                to={path.historyPurchase}
                className='block w-full bg-white py-3 px-4 text-left hover:bg-slate-100 hover:text-cyan-500'
              >
                {t('home:header.my purchase')}
              </Link>
              <button
